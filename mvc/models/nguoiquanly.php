...
class nguoiquanly extends data
{
	...
	// Allow certain file formats
		...
		public function getByKey($key){
			$queryString = "SELECT * FROM `nguoiquanly` LEFT JOIN `huyen` ON (`nguoiquanly`.`ma_huyen` = `huyen`.`ma_huyen`) LEFT JOIN `tinh_tp` ON (`huyen`.`ma_tinhtp` = `tinh_tp`.`ma_tinhtp`) WHERE taikhoan_nql = '$key'";
		...
		public function loginCheck($post_request){
			$taikhoan_nql = $post_request['taikhoan_nql'];
			$matkhau_nql = $post_request['matkhau_nql'];
			$check = "SELECT * 
					  FROM `nguoiquanly` 
